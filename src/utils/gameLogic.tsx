...
export const moveTiles = (
  ...
  const slideAndMerge = (row: number[]): number[] => {
    ...
    for (let i = 0; i < filteredRow.length; i++) {
      if (!skip && i < filteredRow.length - 1 && filteredRow[i] === filteredRow[i + 1]) {
        result.push(filteredRow[i] * 2);
  ...
  const setRows = (rows: number[][]): void => {
    if (direction === 'left' || direction === 'right') {
      rows.forEach((row, i) => row.forEach((val, j) => (newGrid[i * size + j] = val)));
    } else {
      rows.forEach((col, i) => col.forEach((val, j) => (newGrid[j * size + i] = val)));
    }
  };
